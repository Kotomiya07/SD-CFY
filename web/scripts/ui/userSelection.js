...
export class UserSelectionScreen {
	async show(users, user) {
		...
		const userSelection = document.getElementById("sdcfy-user-selection");
		...
		return new Promise((resolve) => {
			...
			const error = userSelection.getElementsByClassName("sdcfy-user-error")[0];
			const button = userSelection.getElementsByClassName("sdcfy-user-button-next")[0];
			...
			if (user) {
				const name = localStorage["sdcfy.userName"];
				if (name) {
					input.value = name;
				}
			}
