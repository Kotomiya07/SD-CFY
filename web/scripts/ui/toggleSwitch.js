...
export function toggleSwitch(name, items, { onChange } = {}) {
	...
	function updateSelected(index) {
		if (selectedIndex != null) {
			elements[selectedIndex].classList.remove("sdcfy-toggle-selected");
		}
		onChange?.({ item: items[index], prev: selectedIndex == null ? undefined : items[selectedIndex] });
		selectedIndex = index;
		elements[selectedIndex].classList.add("sdcfy-toggle-selected");
	}
	...
	const container = $el("div.sdcfy-toggle-switch", elements);
