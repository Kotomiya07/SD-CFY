import { $el, sdcfyDialog } from "../../scripts/ui.js";
...
export class ManageGroupDialog extends sdcfyDialog {
	...
	constructor(app) {
		super();
		this.app = app;
		this.element = $el("dialog.sdcfy-group-manage", {
	...
	show(type) {
		...
		this.innerNodesList = $el("ul.sdcfy-group-manage-list-items");
		this.widgetsPage = $el("section.sdcfy-group-manage-node-page");
		this.inputsPage = $el("section.sdcfy-group-manage-node-page");
		this.outputsPage = $el("section.sdcfy-group-manage-node-page");
		...
		const outer = $el("div.sdcfy-group-manage-outer", [
			$el("header", [
				$el("h2", "Group Nodes"),
				$el(
					"select",
			...
			$el("main", [
				$el("section.sdcfy-group-manage-list", this.innerNodesList),
				$el("section.sdcfy-group-manage-node", [
					$el(
						"header",
			...
			$el("footer", [
				...
				$el("button.sdcfy-btn", { onclick: () => this.element.close() }, "Close"),
			]),
		]);
