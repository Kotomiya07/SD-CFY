...
import { sdcfyDialog, $el } from "../../scripts/ui.js";
import { sdcfyApp } from "../../scripts/app.js";
...
async function uploadMask(filepath, formData) {
	...
	sdcfyApp.clipspace.imgs[sdcfyApp.clipspace['selectedIndex']] = new Image();
	sdcfyApp.clipspace.imgs[sdcfyApp.clipspace['selectedIndex']].src = api.apiURL("/view?" + new URLSearchParams(filepath).toString() + app.getPreviewFormatParam() + app.getRandParam());

	if(sdcfyApp.clipspace.images)
		sdcfyApp.clipspace.images[sdcfyApp.clipspace['selectedIndex']] = filepath;
...
class MaskEditorDialog extends sdcfyDialog {
	...
	createLeftSlider(self, name, callback) {
		...
		divElement.style.backgroundColor = "var(--sdcfy-input-bg)";
			...
			mutations.forEach(function(mutation) {
					if (mutation.type === 'attributes' && mutation.attributeName === 'style') {
						if(self.last_display_style && self.last_display_style != 'none' && self.element.style.display == 'none') {
							...
							sdcfyApp.onClipspaceEditorClosed();
						}
	...
	async setImages(imgCanvas) {
		...
		// image load
		const filepath = sdcfyApp.clipspace.images;

		const alpha_url = new URL(sdcfyApp.clipspace.imgs[sdcfyApp.clipspace['selectedIndex']].src)
		...
		// original image load
		const rgb_url = new URL(sdcfyApp.clipspace.imgs[sdcfyApp.clipspace['selectedIndex']].src);
	...
	async save() {
		...
		if(sdcfyApp.clipspace.images)
			sdcfyApp.clipspace.images[0] = item;

		if(sdcfyApp.clipspace.widgets) {
			const index = sdcfyApp.clipspace.widgets.findIndex(obj => obj.name === 'image');

			if(index >= 0)
				sdcfyApp.clipspace.widgets[index].value = item;
		}
		...
		sdcfyApp.onClipspaceEditorSave();
		this.close();
	}
}

app.registerExtension({
	name: "sdcfy.MaskEditor",
	init(app) {
		...
		const context_predicate = () => sdcfyApp.clipspace && sdcfyApp.clipspace.imgs && sdcfyApp.clipspace.imgs.length > 0
		ClipspaceDialog.registerButton("MaskEditor", context_predicate, sdcfyApp.open_maskeditor);
	}
});
