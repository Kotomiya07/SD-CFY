...
describe("users", () => {
	...
	function expectNoUserScreen() {
		...
		const selection = document.querySelectorAll("#sdcfy-user-selection")?.[0];
		expect(selection["style"].display).toBe("none");
		const menu = document.querySelectorAll(".sdcfy-menu")?.[0];
	...
	describe("multi-user", () => {
		...
		async function testUserScreen(onShown, users) {
			...
			expect(localStorage["sdcfy.userId"]).toBeFalsy();
			expect(localStorage["sdcfy.userName"]).toBeFalsy();
			...
			expect(document.querySelectorAll("#sdcfy-user-selection")).toHaveLength(0);
			expect(window.getComputedStyle(menu)?.display).not.toBe("none");
			...
			if (isCreate) {
				expect(api.storeUserData).toHaveBeenCalledWith("sdcfy.templates.json", null, { stringify: false });
		...
		it("allows user creation if no users", async () => {
			...
			expect(localStorage["sdcfy.userId"]).toBe("Test User!");
			expect(localStorage["sdcfy.userName"]).toBe("Test User");
		});
		it("allows user creation if no current user but other users", async () => {
			...
			expect(localStorage["sdcfy.userId"]).toBe("Test User 3!");
			expect(localStorage["sdcfy.userName"]).toBe("Test User 3");
		});
		it("allows user selection if no current user but other users", async () => {
			...
			expect(localStorage["sdcfy.userId"]).toBe("B!");
			expect(localStorage["sdcfy.userName"]).toBe("B");
		});
		...
		it("allows user switching", async () => {
			...
			expect(app.ui.settings.settingsLookup["sdcfy.SwitchUser"]).toBeTruthy();
		});
	});
	describe("single-user", () => {
		...
		it("doesnt allow user switching", async () => {
			...
			expect(app.ui.settings.settingsLookup["sdcfy.SwitchUser"]).toBeFalsy();
		});
	});
